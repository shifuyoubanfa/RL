...
| `configs/train.yaml` | 全流程唯一配置，含裁判标定表 | 贯穿全篇 |
| `docs/RESULTS.md` | 全部数字的唯一出处：成绩、数据流追溯、标定全表 | 第 12 章 |

**前置术语**：think / answer · 检索腔 · answer-lock · 认可答案池 · 三件套 · 真涨门 · rollout ·
优势 · 参考模型 · LoRA · ZeRO。第一次出现的地方都有一句大白话解释，不用提前查。

> 全篇最值得看的是三处：**7.2**（为什么奖励必须是硬门不是加权和）、**8.4**（一个序列级的
> 分数怎么落到 think 段每一个 token 上）、**12.2**（0.15 这道真涨门是怎么算出来的，
> 以及为什么 +0.155 只能说"刚过门"、余量不到 0.01）。这三处是这条链路和"调框架命令"的
> 分界线。
...
| 显存和分布式 | ✅ | 第 10 章 |
| 三件套评测与统计判据 | ✅ | 第 11 章 |
| 五阶段的实测成绩、真涨门判读 | ✅ | 第 12 章；完整口径见 [RESULTS.md](RESULTS.md) |
| 裁判标定的六档表与可分性矩阵 | ✅ | 摘要在 11.5，全表在 [RESULTS.md](RESULTS.md) 第 5 节 |
| PPO 的 critic / GAE | ⚠️ | 只在第 14 章作为对照提一句。这条链路用的是 GRPO，没有 critic |
| 基座本身怎么微调出来的 | ⛔ | 上游的事，不在这条链路里 |

也就是说：**这份文档讲清楚这条链路的每一个齿轮怎么咬合，以及跑出来那些数该怎么读。**
...
> 在 500 题冻结验收集上，裁判干净分 3.14 → 4.64、规则去检索腔通过率 2.6% → 49.0%，
> 两项都远超统计判据的真涨门。
...
> 梯度就只能压在推理段上。
>
> 结果是在 500 题冻结验收集上，裁判干净分从 3.14 涨到 4.64，规则去检索腔通过率
> 从 2.6% 涨到 49.0%。这里我要说清楚的是判据：干净分有噪声，我先标定过裁判，
> 算出真涨门是 0.15 分，只有超过它才算真涨——四步里只有两步过了门。
> 唯一没收口的是答案在池率 84.6%，差 0.4 个点没回到自己定的 85% 地板。
>
...
当后面要改造的对象和对照。2239 道题切成 500 条冻结验收集 + 1739 条训练集，
验收集全程不参与任何训练和选样。再让基座对每题贪心一次加采样八次，
把这九条答案存成一个"认可池"——后面判"答案漂没漂"就拿它当靶子。

四个阶段实际筛出来的数据量：冷启动 930 条、拒绝采样 204 条、偏好对 885 对、
在线阶段 1668 道可训练题。门越往后越严，因为前面要量、后面要质。
...
干净分有噪声，两阶段差值要过三倍标准误才算真涨——实测这道门是 0.15 分。
五个阶段的成绩和逐步判读见第 12 章。
...
**总账。** 四次转变里只有两次越过了 0.15 那道真涨门：基座→SFT 的 +1.268（同时规则通过率
2.6%→45.4%，主客观同向，排除了只刷代理指标的可能），以及 RFT→DPO 的 +0.155（刚过门）。
DPO 和在线 GRPO 三件套差值全在噪声内，统计打平，最优记 DPO。
三件套整体判 FAIL——答案在池率 84.6%，差 0.4 个点没回 85% 地板。这条我会主动说，
因为验收口径是自己定的，定完就得照它判。
...
这三道门实际跑下来：1739 道训练题过完门，攒到 **930 条**（837 条进训练、93 条确定性留出
当验证集）。目标数配的是 700，攒够即停——所以最后那一批多跑了一点，落在 930。
后面两个阶段的门更严，拒绝采样筛出 204 条、偏好对筛出 885 对。
...
这张表是面试里最容易被追问细节的地方，数字要能张口就来。

| 阶段 | 数据怎么来 | 实得 | 训练配置 | 优化步数 |
| --- | --- | ---: | --- | ---: |
| 冷启动 SFT | 裁判改写 1739 题 → 三道门 | 930 条<br/>（837 训 + 93 留出验证） | lr 5e-5，7 轮 | ≈ 100 |
| 拒绝采样 | 每题自采 32 条 → 三道门 | 204 条 | lr 3e-5，3 轮 | — |
| DPO | 每题采 16 条 → 构 answer-lock 对子 | 885 对<br/>（3σ 桶另有 478 对） | β 0.1，lr 5e-6，2 轮，<br/>rpo_alpha 1.0，等效批量 16 | 108 |
| 在线 GRPO | 1668 道可训练题，组大小 K=8 | — | 预热 β 0.08 / 在线 β 0.06，<br/>lr 7e-7 | 30 + 90 |

LoRA 配置四阶段共用：秩 16、α 32、dropout 0.05，挂在七个投影矩阵上。

**三个要解释得出来的取舍：**

1. **各阶段的门松紧不一样，是刻意的。** 冷启动要**量**——要足够多的干净种子把写法教会，
   所以门相对松（930 条）；拒绝采样和偏好对要**质**——要高质量梯度信号，所以门严
   （204 条 / 885 对）。
2. **在线阶段剔掉了 71 道题**（1739 → 1668）。这些题的认可池里抽不出任何可比较的
   极性/数字/日期，模型答什么都判不了在不在池。留着它们只会教模型"少说具体数字"。
3. **DPO 那 108 步是算出来的**：885 对 ÷ 等效批量 16 × 2 轮 ≈ 108。
   一轮只有五十来个优化步，所以轮数不能给太小——不然学习率调度器还没热身完就结束了。

### 5.4 有没有 critic、有没有 reward model
...
代入本项目标定量出来的两个数（σ_judge = 0.74、σ_between = 1.02）和主线验收口径
（N=500、k=3）：

```
SE = √(1.02² + 0.74²/3) / √500 ≈ 0.0495
真涨门 = 3 × SE = 0.1484，全文按 0.15 引用
```

**大白话：500 道题、每题打 3 遍，这套评测能稳定读出来的最小差异就是 0.15 分。**
比这小的差，读不出来。
...
做法：取 13 个参考问答对，每个人工做出 6 档已知脏度的推理段——"抄 0 句 / 1 句 / 2 句 /
3 句 / 4 句 / 整段抄"，共 78 条；让裁判对每一条各打 16 遍，合计 1248 次判分调用。
16 遍的均值就是该档的分数，16 遍的标准差就是该档的档内噪声。

量出来的表：

| 脏度档 | 人为理想锚点 | **实测均值** | **档内 σ** |
| --- | ---: | ---: | ---: |
| 没抄 | 10 | **7.34** | 1.85 |
| 抄 1 句 | 8 | **4.64** | 1.63 |
| 抄 2 句 | 6 | **2.64** | 0.60 |
...
1. **整条梯子被系统性压低。** 连"完全没照抄"裁判也只给 7.34，不给满分。但单调性还在
   （Spearman 等级相关 0.954），所以它能当**相对**尺子用，不能当**绝对**分数读。
2. **越靠干净端 σ 越大。** 按 3σ 带不相交的判据算，6 个相邻档里**只有"抄 3 句↔抄 4 句"
   那一对可分**；"没抄↔抄 1 句"的 σ 距离只有 0.8，完全分不开。
3. **多打压不掉它。** "没抄"档从 k=1 打到 k=16，观测标准差只从 2.13 降到 1.85。

第 2 条直接决定了偏好对的造法——chosen 和 rejected 必须是"洗干净的"对"基座原推理"，
不能是"洗干净的"对"稍微没那么干净的"，后者落在裁判分不开的区间里，学出来的是噪声。
第 3 条决定了整体评测靠加题数不靠加 k。

完整的可分性矩阵见 [RESULTS.md](RESULTS.md) 第 5 节。
...
评测的尺子讲完了，下面用它去读这条链路真正跑出来的数。

---

## 第12章 最终成绩与怎么读这些数

全部数字的唯一出处是 [RESULTS.md](RESULTS.md)。这一章讲的是**怎么读**它们——
面试里真正会被追问的不是"涨了多少"，是"你凭什么说这是涨了"。

### 12.1 主线五阶段三件套

同一套冻结的 500 题验收集，每个数字都标了谁评的、样本量多少。
...
### 12.2 真涨门：0.15 是怎么算出来的

干净分是裁判打的，有噪声。所以两个阶段之间的差值必须先过一道门，才敢说涨了。

代入标定量出来的两个数（σ_judge = 0.74、σ_between = 1.02）：
...
二值比例的标准误：规则通过率约 2.2 个百分点，在池率约 1.6 个百分点。

这张图要说明的是：把真涨门画成一条刻度，五阶段的四次转变里哪几次真的越过去了。

```mermaid
flowchart LR
    B["基座<br/>3.140"] -->|"+1.268 ✅ 过门"| S["SFT<br/>4.408"]
    S -->|"+0.081 ⛔ 没过"| R["RFT<br/>4.489"]
    R -->|"+0.155 ✅ 刚过门"| D["DPO<br/>4.643"]
    D -->|"−0.042 ⛔ 打平"| G["GRPO<br/>4.601"]
```

*图11：四次转变里只有两次越过了 0.15 这道门*

从图里该看出三件事：

1. **第一跳最大，后面全在门槛附近走** ——+1.268 是把"自然推理这种写法"教进去的一次性收益。
   之后每一步的增量都贴着门槛，这本身就是"该用统计判据而不是看数字大小"的理由。
2. **+0.155 越过门槛的余量不到 0.01 分** ——门槛未取整是 0.1484，余量 0.0066。
   这是全篇最该老实说的一个数：它过了门，但过得很勉强。面试里被追问"这个 +0.155 稳不稳"，
   正确答案是"按 3σ 判据它是真涨，但余量不到 0.01，我不会拿它去论证 DPO 比 RFT 强很多"。
3. **最后一步是负的，但仍记打平** ——−0.042 的绝对值远小于 0.15，方向是负的也读不出来。
   不能说"GRPO 退步了"。

### 12.3 三件套要一起读，单看一列会得出反的结论

同一张表，只盯一列会看出三个不同的"最优阶段"：

| 只看这一列 | 最优是谁 | 单看它的结论 |
|---|---|---|
| 裁判干净分 | DPO（4.643） | DPO 最好 |
| 规则去检索腔通过率 | GRPO（50.0%） | GRPO 最好 |
| 答案在池率 | 基座基线（93.8%） | **什么都别训最好** |

第三行是关键：**在池率的最高点是不训练。** 因为这个指标衡量的是"答案有没有跟基座不一样"，
原地不动当然满分。它是护栏不是升力，只能用来看"有没有塌"，绝不能用来选最优阶段。

前两列的最高点也不在同一个阶段，因为它们判的是两件不同的事：规则查的是**表面检索腔词**，
裁判查的是**换词复述照抄**。表面词的清除主要发生在第一步（2.6% → 45.4%），
之后只在 45%~50% 之间小幅爬；而换词复述那一维一直在缓慢改善。

综合三列取 DPO：它在有噪声的那一列上是最高点，另外两列都和最高点统计打平。

### 12.4 格式失败要单独记账，不能混进主指标

DPO 和 GRPO 各有几道题的推理段撞上生成长度上限，`</think>` 没闭合、答案为空。
把它们剔掉重算：
...
格式失败题数：DPO 1 道、GRPO 2 道。**结论不翻盘**——剔干净后 DPO 仍略高于 GRPO，
两者仍都在 85% 地板之下。

做这一步复核的理由是**先堵住一条质疑**：有人会说"你那 0.8 个百分点的差距，是不是就是
多了一道格式失败题？"。答对这个问题需要提前算好，不能临场估。

### 12.5 DPO 和 GRPO 是统计打平，不是 GRPO 更差

| 指标 | DPO | GRPO | 差值 | 噪声门槛 | 判读 |
|---|---:|---:|---:|---|---|
| 裁判干净分 | 4.643 | 4.601 | −0.042 | 3×SE ≈ 0.15 | 持平 |
| 规则通过率 | 49.0% | 50.0% | +1.0pp | 二值 SE ≈ 2.2pp | 在噪声内 |
| 答案在池率 | 84.6% | 83.8% | −0.8pp | 二值 SE ≈ 1.6pp | 在噪声内 |

三个差值全部小于各自门槛 = **统计上打平**。最优仍记 DPO，理由是它在有噪声的那一列上
数值更高，而不是因为它"显著更好"。

**在线阶段为什么守住了却推不动**，三条原因都能指到具体机制：

1. **表面规则那一档已经没有梯度可给。** 到 DPO 规则通过率已经 49.0%，
   在线奖励里"规则档位"面对的大部分候选都已经无痕，区分不开。
2. **剩下要推的换词复述正落在裁判分辨率最差的一档。** 候选大都已经挺干净，
   而标定表说得很死：干净端 σ≈1.85、相邻档分不开。在线用 k=2 在一组都挺干净的候选里
   排不出可靠高低。
3. **答案在池是硬门，只防漂不上拉。** 它是约束不是升力，不会把在池率从 84% 主动顶到 85%。

所以在线阶段的定位是**守住增益 + 确认天花板**，这个天花板的位置不在 RL 算法，
而在裁判在干净端的分辨率。

### 12.6 最终验收：三件套整体判 FAIL
...
**这条要主动说，不要等人问出来。** 三件套是自己定的验收口径，定完之后就得照它判，
不能因为两件过了就说项目成功。
...
这 0.4 点是一个明确的、可以继续工程化的剩余问题，不是路线问题。收口的方向有两个：
在线阶段把在池率从约束变成升力；或者在评测侧把"无可比事实"那部分单独收口。

### 12.7 一道题的推理段在五个阶段里怎么变

同一道题、同一份参考资料，看推理段变了什么、答案有没有跟着变。
...
基座的推理段开头是"根据参考问答对"，逐条标"（参考问答对1）"，结尾还显式列
"推理过程显式引用来源：步骤1来自参考问答对1和3……"。SFT 之后变成：
...
1. **答案全程锁死。** 五个阶段的答案结论都是"20% 股息红利 + 建议跟主管税务机关确认"，
   和基座原版一致。这是 answer-lock 的直接证据。
2. **单条裁判分很粗，别拿它说事。** 除基座（1.33）外其余四个阶段都是 2.0，
   单条几乎分不出谁更干净。真正一眼能看出去检索腔的是**规则**判定（有痕→无痕）；
   裁判干净分必须靠**整批 500 条的群体均值**（3.140 → 4.643）才准。
   这就是"群体均值能读、单条读不出"——单条受 σ_between 主导，群体靠 √N 摊薄掉了。

### 12.8 三套口径不许横向比

| 口径 | 谁评的 | 样本量 | 代表数字 | 地位 |
|---|---|---:|---|---|
| **三件套** | 裁判(k=3) + 规则 | **N=500** | 干净分 3.14 → 4.64 | ✅ 最终采用 |
| 纯规则痕迹率 | 全规则正则 | N=224 | clean_rate ≈ 0.24 | 未采用 |
| 相似度型 humanness | 裁判单次 | N=224 | 0.212 → 0.846 | 已弃，仅立项对照 |

判的不是同一件事、量纲不同、样本集不同、裁判口径也不同。所以
**"humanness 0.846"和"干净分 4.64"之间不构成任何比较关系**。
面试里被拿这两个数问"是不是退步了"，答案是"这是两把不同的尺子，不能比"。

成绩讲完了，下面把工程上真正难的几处单列出来。
...
### 14.16 什么是 reward hacking，这条链路怎么防
...
> 先量再用。开训之前取 13 个参考问答对，每个造 6 档已知脏度的推理段，共 78 条，
> 每条让裁判各打 16 遍，合计 1248 次调用，量出每档的均值和档内标准差。
>
> 量出来三条。第一，整条梯子被系统性压低——连"完全没照抄"裁判也只给 7.34 分不给满分，
> 但单调性还在，Spearman 0.954，所以它能当相对尺子不能当绝对分数。
> 第二，越靠干净端 σ 越大："没抄"档 σ≈1.85，"抄 4 句"档 σ≈0.06；按 3σ 带不相交算，
> 6 个相邻档里只有一对可分。第三，多打压不掉——"没抄"档从 k=1 到 k=16，
> 观测标准差只从 2.13 降到 1.85。
>
> 这三条各自有用处。第二条决定了偏好对必须是"洗干净的"对"基座原推理"，
> 不能是两条都挺干净的。第三条决定了整体评测靠加题数不靠加 k。
> 代入 σ_judge=0.74、σ_between=1.02，N=500/k=3 时标准误约 0.05，真涨门就是 0.15。

### 14.20 为什么评测的 k 只有 3，选样却要 16
...
> 在 500 题冻结验收集上，三件套是这样的：裁判 think 干净分 3.140 → 4.643，
> 规则去检索腔通过率 2.6% → 49.0%，答案在池率 93.8% → 84.6%。
>
> 判据我先说清楚：干净分是裁判打的、有噪声，我标定过它，算出 N=500 / k=3 时
> 标准误约 0.05，真涨门 3×SE ≈ 0.15。四次阶段转变里只有两次越过了这道门——
> 基座到 SFT 的 +1.268，和 RFT 到 DPO 的 +0.155。另外两次都在噪声带内，记统计打平。
>
> 另外两个指标是确定性规则算的，没有打分噪声，直接比数就行。

### 14.22 为什么在线 GRPO 没有超过 DPO

> 先摆事实：三件套差值全部落在各自噪声门槛内，是**统计打平**，不是 GRPO 更差。
> 干净分差 −0.042，门槛 0.15；规则通过率 +1.0 个点，二值标准误约 2.2 个点；
> 在池率 −0.8 个点，标准误约 1.6 个点。
>
> 推不动的原因有三条，都能指到机制。第一，表面规则那一档到 DPO 已经 49% 通过，
> 在线奖励面对的大部分候选都已经无痕，给不出区分度。第二，剩下要推的换词复述
> 正落在裁判分辨率最差的一档——候选大都挺干净了，而标定表说得很死：
> 干净端 σ≈1.85、相邻档分不开，在线用 k=2 排不出可靠高低。第三，答案在池是硬门，
> 它是约束不是升力，不会把在池率从 84% 主动顶到 85%。
>
> 所以在线阶段的定位是守住增益并确认天花板。这个天花板的位置不在 RL 算法，
> 在裁判于干净端的分辨率。

### 14.23 三件套整体判 FAIL，那这个项目算成功吗

> 按我自己定的验收口径，判 FAIL，唯一卡点是答案在池率 84.6%，差 0.4 个点没回 85% 地板。
> 这条我会主动说——口径是开训前定死的，定完就得照它判，不能因为两件过了就说成功。
>
> 但要把两件事分开。**方法上是成立的**：干净分和规则通过率都相对基线大幅真涨，
> 而且是主客观同向，排除了只把代理指标刷高。**工程上还差一步**：那 0.4 个点是一个明确的、
> 可以继续做的剩余问题。
>
> 掉的原因也清楚：answer-lock 锁的是**训练样本里的答案文本**，推理时模型是现吐一段
> 新答案的。把推理段推干净的策略顺带把答案分布也推偏了一点。收口方向有两个——
> 在线阶段把在池率从约束改成升力，或者在评测侧把"无可比事实"那部分单独收口。

### 14.24 那个 +0.155 越过门槛的余量不到 0.01，这个结论稳吗

> 按 3σ 判据它算真涨——门槛未取整是 0.1484，余量 0.0066——但我不会拿它去论证
> "DPO 比拒绝采样强很多"。它过门过得很勉强。
>
> 有两条旁证让我更愿意信它。一是规则通过率同步从 45.6% 涨到 49.0%——那是确定性指标、
> 没有打分噪声，主客观同向就不太可能是裁判抖出来的。二是剔掉格式失败题重算，
> 干净分从 4.643 变成 4.637，几乎没动，说明这个差不是几道异常样本撑起来的。
>
> 要把这个结论做得更硬，正确的做法是加题数不是加 k——标准误按 √N 缩，
> 而加 k 几乎不动，这一条在标定的时候就量清楚了。
...
### 14.26 这个项目怎么迁移到 Agent 或 RAG 场景
...
> 四处，按重要性排。
>
> 第一，**答案在池率还差 0.4 个点没收口**（84.6% 对 85% 地板），三件套整体判 FAIL。
> 这是唯一一条硬指标没达成的，也是最该继续做的。
>
> 第二，**裁判的系统性偏差没测**。标定表量的是方差，不是偏差。裁判如果对某类题材一律偏高，
> 表里看不出来，得换一批标定样本才测得到。而现在的天花板恰恰卡在裁判的分辨率上，
> 这块没量清楚就没法判断"再往上还有多少空间"。
>
> 第三，**LoRA 的秩和 alpha 没做消融**，用的是常见起点配置。
>
> 第四，**长度和重复的惩罚是补丁**，只处理了已经见过的两种退化形态。
...
> 基于 ms-swift + DeepSpeed ZeRO-3 + LoRA + vLLM，覆盖数据构建、训练编排到离线评测的
> 端到端可复跑流程；在 500 题冻结验收集上把推理段裁判干净分从 3.14 提升到 4.64、
> 规则去检索腔通过率从 2.6% 提升到 49.0%。**

> **设计词典序硬门奖励函数并接入 GRPO：格式、答案漂移、检索腔三道确定性规则门在前，
> 大模型裁判只在安全区内做组内排序，从机制上堵死"推理写得漂亮补偿答案漂移"这条
> reward hacking 路径；配合 answer-lock 偏好对（885 对，chosen/rejected 答案段逐字相同）
> 使梯度只作用于推理段。**

> **开训前先把裁判当测量仪器标定：造 78 条已知脏度样本 × 16 次重复判分，
> 量出逐档打分噪声，据此定义 σ-可分选样门与 3×SE≈0.15 的真涨门；
> 全部阶段结论按该判据判读，四次阶段转变中仅两次判定为统计显著提升。**
...
> 推理写得再好也补不回来。
>
> 结果是 500 题冻结验收集上裁判干净分 3.14 涨到 4.64、规则去检索腔通过率 2.6% 涨到 49.0%。
> 判据是开训前标定出来的真涨门 0.15 分，四次阶段转变里只有两次过了门。
> 有一条没收口：答案在池率 84.6%，差 0.4 个点没回我自己定的 85% 地板，整体判 FAIL。
>
...
| `docs/RESULTS.md` | 全部数字的唯一出处：成绩、数据流追溯、标定全表 |
...
| ✅ 五阶段三件套成绩已量并按统计判据判读 | N=500 冻结验收集，裁判 k=3；真涨门 3×SE≈0.15；见第 12 章与 [RESULTS.md](RESULTS.md) |
| ✅ 裁判分辨率已标定 | 78 条已知脏度样本 × 16 次重复，六档均值与档内 σ 写进 `configs/train.yaml` |
| ⚠️ 三件套整体判 FAIL | 答案在池率 84.6% < 自定的 85% 地板，差 0.4 点。干净分与规则通过率两件已达成 |
| ⚠️ RFT→DPO 那 +0.155 越门余量不到 0.01 | 门槛 0.1484，余量 0.0066。按 3σ 判据算真涨，但过得勉强 |
| ⚠️ 本仓库不含实验产物本身 | 成绩数字在文档里，但原始 `summary.json`、权重、语料都不入库 |
| ⚠️ 完整链路的真机复跑不在本仓库的验证范围内 | 它需要一个 32B 领域基座、多卡机器、一份领域语料、一个裁判服务 |
